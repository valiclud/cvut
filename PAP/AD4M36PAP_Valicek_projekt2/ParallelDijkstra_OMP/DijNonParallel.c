...
{  int j;  
    ...
    if ((ftp = fopen("Test100.txt","r")) == NULL){
...
void findMin(unsigned *d, int *v)  
{  int i;  
   *d = largeint;   
   for (i = 0; i <= vertices; i++)  
      if (flag[i] && minDistance[i] < largeint)  {  
         *d = graph[i];  
         *v = i;  
      }  
}    
...
{    
         int step, 
          minVertex;  // min hodnota vrcholu 
          unsigned minValue;  // naleyen8 minim8ln9 hodnota
          
      for (step = 0; step < vertices; step++)  {  
           minimum = largeint; 
           minimumVertex = 0;    
           findMin(&minValue,&minVertex);   
             if (minValue < minimum)    
               {  
                 minimum = minValue; minimumVertex = minVertex;  
             }  
